...
X_training=[X2_training,X5_training ...
           X7_training,X9_training];

X_test=[X2_test,X5_test ...
           ,X7_test,X9_test];
...
label_train(label_train==5)=2;
label_train(label_train==7)=3;
label_train(label_train==9)=4;

label_test(label_test==2)=1;
label_test(label_test==5)=2;
label_test(label_test==7)=3;
label_test(label_test==9)=4;
...
X_train_std = repmat(X_train_std,[1 320]);
X_train_1 = X_train_1-repmat(X_train_mean,[1 320]);
X_train_1 = X_train_1./X_train_std;

hidden_vector =[]; train_error =[]; test_error =[];
...
for i = 1:17
    ...
    X_train_std = repmat(X_train_std,[1 80]);
    X_test_1 = X_test_1-repmat(X_train_mean,[1 80]);
...
% training_error
    ...
    hidden_vector = [hidden_vector;hidden_num];
    train_error = [train_error;train_e];
    test_error = [test_error;test_e];
